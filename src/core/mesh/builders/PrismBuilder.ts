...
export function buildPrism(options: PrismOptions = {}): EditableMesh {
  const {
    radius = 0.5,
    height = 1,
    radialSegments = 6,
  ...
  const segs = Math.max(3, Math.min(32, Math.floor(radialSegments)));
  ...
  const capUv = (i: number) =>
    v2(
      0.5 + 0.5 * Math.cos((i / segs) * Math.PI * 2 - Math.PI / 2),
      0.5 + 0.5 * Math.sin((i / segs) * Math.PI * 2 - Math.PI / 2),
    );
...
export const PrismBuilder: PrimitiveBuilder<PrismOptions> = {
  id: 'prism',
  label: 'Prism',
  defaultOptions: { radius: 0.5, height: 1, radialSegments: 6, name: 'Prism' },
  build: buildPrism,
};
